    ...
    useEffect(() => {
        const initData = async () => {
            ...
            try {
                ...
                // Bước 1: Lấy thông tin Store từ User ID đang login
                const storeRes = await getStoreByUserId(user.id);
                
                // Kiểm tra nếu có dữ liệu store (lấy storeRes.data dựa trên cấu trúc JSON bạn cung cấp)
                const storeData = storeRes?.data || storeRes;

                if (storeData && storeData.id) {
                    // Bước 2: Dùng store_id vừa lấy được để fetch sản phẩm
                ...
                // Nếu lỗi 404 từ getStoreByUserId, thông báo người dùng chưa có store
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-orange-50/50 border-b border-gray-100">
                                ...
                                <th className="p-4 font-bold text-gray-400 text-[11px] uppercase w-24">Hình ảnh</th>
                                ...
                                <th className="p-4 font-bold text-gray-400 text-[11px] uppercase text-center">Ngày đăng</th>
                            ...
                            {loading ? (
                                <tr>
                                    <td colSpan="6" className="p-10 text-center">
                                        ...
                                        <p className="text-xs text-gray-400 mt-2">Đang tải dữ liệu kho hàng...</p>
                                    </td>
                                </tr>
                            ) : products.length === 0 ? (
                                <tr>
                                ...
                                products.map((prod, index) => {
                                    ...
                                    return (
                                        <tr key={prod.id} className="hover:bg-orange-50/20 transition-colors">
                                            ...
                                            <td className="p-4 text-center">
                                                <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase ${prod.status === 1 ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`}>
                                                    {prod.status === 1 ? 'Đang hoạt động' : 'Đã ẩn'}
                                                </span>
                                            </td>
                                            <td className="p-4 text-center text-sm text-gray-500 font-medium">
                                                {new Date(prod.timestamp || prod.created_at).toLocaleDateString('vi-VN')}
                                            </td>
                                            <td className="p-4">
