...
import { ShoppingCart, Heart, Truck, RotateCcw, Minus, Plus, Check, Star, User, ChevronDown } from 'lucide-react';
...
function ProductDetail() {
    ...
    const handleAddToCart = async () => {
        if (!isLoggedIn) { alert("Vui lòng đăng nhập!"); return navigate('/login'); }
        if (!selectedVariant) return alert("Vui lòng chọn phiên bản!");
        if (quantity > selectedVariant.quantity) return alert("Số lượng vượt quá tồn kho!");
        ...
        try {
            ...
            if (res.success) navigate('/gio-hang');
        } catch (error) {
    ...
    return (
        <div className="bg-white min-h-screen pb-20 pt-28">
            <div className="container mx-auto px-4">
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-5 gap-3">
                            {product.images?.map((img) => (
                                <div key={img.id} onClick={() => setMainImage(img.image)} className={`aspect-square cursor-pointer border-2 transition-all p-1 ${mainImage === img.image ? 'border-sky-500' : 'border-transparent hover:border-gray-200'}`}>
                                    <img src={`${CLOUDINARY_PRODUCT}/${img.image}`} className="w-full h-full object-cover" alt="" />
                                </div>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col">
                        <div className="mb-6">
                            ...
                            <div className="flex items-center space-x-4 mb-2">
                                <div className="flex items-center text-yellow-400">
                                    {[...Array(5)].map((_, i) => (
                                        <Star key={i} size={16} fill={i < Math.round(stats.avg) ? "currentColor" : "none"} />
                                    ))}
                                    <span className="ml-2 text-sm font-bold text-gray-600">{stats.avg} ({stats.total} đánh giá)</span>
                                </div>
                            </div>
                        ...
                        <div className="bg-gray-50 p-6 mb-8 border-l-4 border-sky-500">
                            <div className="flex items-baseline space-x-3">
                                <span className="text-4xl font-black text-sky-600">{Math.round(finalPrice).toLocaleString('vi-VN')}đ</span>
                                {selectedVariant?.discount > 0 && (
                                    <span className="text-lg text-gray-400 line-through">{Number(selectedVariant.price).toLocaleString('vi-VN')}đ</span>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="mb-8">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                {product.variants?.map((v) => (
                                    <button key={v.id} onClick={() => { setSelectedVariant(v); setQuantity(1); }} disabled={v.quantity === 0}
                                        className={`relative flex flex-col p-4 border-2 transition-all text-left ${selectedVariant?.id === v.id ? 'border-black bg-black text-white' : v.quantity === 0 ? 'opacity-50 cursor-not-allowed' : 'hover:border-gray-300'}`}>
                                        ...
                                        <span className="text-[10px] font-bold uppercase">{v.quantity > 0 ? `Kho: ${v.quantity}` : 'Hết hàng'}</span>
                                    </button>
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="flex items-stretch space-x-4 mb-10 h-14">
                            <div className="flex items-center border-2 border-gray-200">
                                <button onClick={() => setQuantity(q => Math.max(1, q - 1))} className="px-4 h-full hover:bg-gray-100"><Minus size={14} /></button>
                                <span className="w-10 text-center font-black text-lg">{quantity}</span>
                                <button onClick={() => setQuantity(q => q + 1)} className="px-4 h-full hover:bg-gray-100"><Plus size={14} /></button>
                            </div>
                            <button onClick={handleAddToCart} disabled={isAdding || !selectedVariant || selectedVariant.quantity === 0}
                        ...
                        <div className="grid grid-cols-1 gap-4 py-8 border-t border-gray-100">
                            <div className="flex items-center space-x-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-[11px] text-gray-500">Miễn phí vận chuyển từ 1.000.000đ</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div className="mt-24">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                        {product.descriptions?.map((desc) => (
                            <div key={desc.id} className="group border border-gray-100 p-8 hover:border-sky-500 transition-all">
                                <h3 className="text-xl font-black mb-4 text-gray-900 group-hover:text-sky-500 uppercase tracking-tight">{desc.title}</h3>
                                <div className="w-10 h-1 bg-gray-200 mb-4 group-hover:w-20 group-hover:bg-sky-500 transition-all"></div>
                                <p className="text-gray-600 leading-relaxed text-sm font-medium">{desc.content}</p>
                            </div>
                        ))}
                    </div>
                </div>
                ...
                <div className="mt-32">
                    <div className="inline-block border-b-4 border-black pb-2 mb-12">
                        <h2 className="text-3xl font-black uppercase tracking-tighter">Khách hàng nói gì?</h2>
                    </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 mb-16 bg-gray-50 p-10 border border-gray-100">
                        <div className="text-center lg:border-r border-gray-200">
                            ...
                            <div className="flex justify-center text-yellow-400 mb-2">
                                {[...Array(5)].map((_, i) => <Star key={i} size={24} fill={i < Math.round(stats.avg) ? "currentColor" : "none"} />)}
                            </div>
                            <p className="text-xs font-bold text-gray-400 uppercase tracking-widest">{stats.total} lượt đánh giá</p>
                        </div>
                        
                        <div className="lg:col-span-2 flex flex-wrap gap-3 items-center justify-center lg:justify-start">
                            <button onClick={() => setFilterStar(0)} className={`px-6 py-2 rounded-full font-bold text-xs uppercase transition-all ${filterStar === 0 ? 'bg-black text-white' : 'bg-white border border-gray-200 hover:border-black'}`}>Tất cả</button>
                            {[5, 4, 3, 2, 1].map(s => (
                                <button key={s} onClick={() => setFilterStar(s)} className={`px-6 py-2 rounded-full font-bold text-xs uppercase flex items-center space-x-2 transition-all ${filterStar === s ? 'bg-sky-500 text-white' : 'bg-white border border-gray-200 hover:border-sky-500'}`}>
                                    <span>{s}</span> <Star size={12} fill="currentColor" />
                                    <span className="text-[10px] opacity-60">({stats.counts[s]})</span>
                                </button>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="space-y-12">
                        {filteredReviews.length > 0 ? (
                            filteredReviews.slice(0, visibleCount).map((rev) => (
                                <div key={rev.id} className="flex flex-col md:flex-row gap-6 pb-12 border-b border-gray-100 last:border-0">
                                    {/* User Info */}
                                    <div className="w-full md:w-48 shrink-0">
                                        <div className="flex items-center space-x-4 md:flex-col md:items-start md:space-x-0">
                                            <div className="w-12 h-12 md:w-16 md:h-16 rounded-full overflow-hidden border-2 border-gray-100 mb-3">
                                                <img src={rev.user_avatar ? `${CLOUDINARY_USER}/${rev.user_avatar}` : '/default-avatar.png'} className="w-full h-full object-cover" alt="" />
                                            </div>
                                            <div>
                                                <p className="font-black text-sm uppercase tracking-tight text-gray-900">{rev.user_name}</p>
                                                <p className="text-[10px] text-gray-400 font-bold uppercase">{new Date(rev.timestamp).toLocaleDateString('vi-VN')}</p>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex-1">
                                        <div className="flex text-yellow-400 mb-3">
                                            {[...Array(5)].map((_, i) => <Star key={i} size={14} fill={i < rev.point ? "currentColor" : "none"} />)}
                                        </div>
                                        <p className="text-gray-700 leading-relaxed mb-6 font-medium italic">"{rev.content}"</p>
                                        
                                        {/* Review Images */}
                                        {rev.review_images.length > 0 && (
                                            <div className="flex flex-wrap gap-3">
                                                {rev.review_images.map((img, idx) => (
                                                    <div key={idx} className="w-24 h-24 border border-gray-100 p-1 hover:border-sky-500 transition-all cursor-zoom-in">
                                                        <img src={`${CLOUDINARY_REVIEW}/${img}`} className="w-full h-full object-cover" alt="review" />
                                                    </div>
                                                ))}
                                            </div>
                                        )}
                            ...
                            <div className="text-center py-20 bg-gray-50 border-2 border-dashed border-gray-200">
                                <p className="font-bold text-gray-400 uppercase tracking-widest text-sm">Chưa có đánh giá nào phù hợp</p>
                            </div>
                        )}
                    </div>
