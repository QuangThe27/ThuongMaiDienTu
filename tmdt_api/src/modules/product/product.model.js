...
const findDescriptionsByProductId = async (productId) => {
    const [rows] = await db.execute('SELECT * FROM product_descriptions WHERE product_id = ?', [productId]);
    return rows;
};

const findVariantsByProductId = async (productId) => {
    const [rows] = await db.execute('SELECT * FROM product_variants WHERE product_id = ? ORDER BY level ASC', [productId]);
    return rows;
};
...
const insertVariants = async (productId, variants) => {
    const values = variants.map(v => [
        productId, v.variant_name, v.variant_value, v.price, v.quantity, v.discount, v.level
    ]);
    const [result] = await db.query(
